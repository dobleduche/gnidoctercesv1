...
import { AgentStatus, GenerationState, AppGenerationResult, AIModel, Commit, RefinedPrompt, TierId } from '../types';
...
const INITIAL_AGENTS: AgentStatus[] = AGENT_DEFINITIONS.map(agent => ({
    ...
    status: 'pending',
    retries: 0,
    subtasks: agent.subtasks ? agent.subtasks.map(subtask => ({ ...subtask, status: 'pending' })) : undefined,
}));
...
export const useAppGenerator = ({ selectedTier, selectedModel, setSelectedModel: _setSelectedModel }: UseAppGeneratorProps) => {
  ...
  const [scaffoldedProject, setScaffoldedProject] = useState<{ projectId: string; projectDir: string, fileTree: any, files: any } | null>(null);
  ...
  const fetchAvailableModels = useCallback(async () => {
    try {
      ...
      if (models.length > 0 && !models.includes(selectedModel)) {
        ...
        setNotification({
            message: "No AI models configured. Please add an API key to your .env file to enable generation.",
            type: 'warning'
        });
      }
    } catch (error) {
    ...
    try {
        ...
        pollingRef.current = window.setInterval(async () => {
            try {
                ...
                stopPolling();
                setGenerationState(GenerationState.ERROR);
                setNotification({ message: pollError instanceof Error ? pollError.message : "Lost connection to build server.", type: 'error' });
            }
        }, 2000);
