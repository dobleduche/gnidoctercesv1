  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <span className="text-lg font-orbitron font-bold text-cyan" aria-hidden="true">{roundedProgress}%</span>
      </div>
            ...
            return (
              ...
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 border-dark-bg transition-all duration-300 ${agent.status === 'pending' ? 'bg-gray-600' : config.bgColor}`}>
                  <Icon className={`h-4 w-4 text-white ${isRunning || isRetrying ? 'animate-spin' : ''}`} aria-hidden="true" />
                </div>
                <div 
