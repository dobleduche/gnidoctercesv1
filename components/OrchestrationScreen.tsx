...
const buildFeatures = (tierData: {
  models: number;
  media: number;
  lifetime: boolean;
  compliance: boolean | string;
  runway: boolean;
}): { features: { text: string; tooltip: string }[]; limitations: string[] } => {
  ...
  if (tierData.lifetime) {
    features.push({
      ...
      tooltip: 'Pay once and get access to all features of this tier forever.',
    });
  }

  features.push({
    text: `${tierData.models}+ AI Models`,
    tooltip: `Leverages ${tierData.models} AI models competing to produce higher quality, more robust code.`,
  });
  ...
  if (tierData.compliance) {
    ...
    features.push({
      text: complianceText[complianceKey],
      tooltip: complianceTooltip[complianceKey],
    });
  } else {
  ...
  if (tierData.runway) {
    features.push({
      text: 'Runway Program Access',
      tooltip: 'Access to our startup program, including credits and mentorship.',
    });
  } else {
...
export const TIERS: Tier[] = [
  {
    ...
    descriptionTooltip:
      'Uses one powerful AI model for code generation, ideal for startups and individual developers.',
  ...
  },
  {
    ...
    descriptionTooltip:
      'Leverages multiple AI models competing to produce higher quality, more robust code for professional applications.',
  ...
  },
  {
    ...
    descriptionTooltip:
      'A one-time purchase for lifetime access to Foundry tier features. Perfect for serial entrepreneurs and agencies.',
  ...
  },
  {
    ...
    descriptionTooltip:
      'The ultimate tier for enterprise-grade applications, featuring our most advanced models and full compliance support.',
    ...
    >
      <div className={`p-4 ${tier.bgColor}/20`}>
        <div className="flex items-center gap-4 bg-dark-secondary/50 p-3 rounded-lg">
          ...
          {isCurrent && (
            <div
              className="ml-auto h-8 w-8 rounded-full bg-success flex items-center justify-center"
        ...
        <button
          ...
          className={`w-full py-3 rounded-lg font-bold text-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
            isCurrent
              ? 'bg-white/20'
              : isUpgradeAction
                ? `${tier.bgColor}/80 hover:${tier.bgColor} text-white`
                : 'bg-white/20'
          }`}
        >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-8">
          <header className="text-center mb-8">
            ...
            <p className="text-text-muted mt-2 max-w-3xl mx-auto">
              All paid plans generate real deploy-ready code. Higher tiers unlock more AI-models,
              media generation, compliance, and lifetime access.
            </p>
          </header>
