  ...
  return (
    ...
    >
      ...
      <div className="relative w-16 h-16 flex items-center justify-center">
        ...
        <div
          className={`absolute w-12 h-12 rounded-full flex items-center justify-center ${color}`}
        >
          <AnimatePresence mode="wait">
            ...
            >
              <Icon
                className={`h-7 w-7 ${isRunning || isRetrying ? 'animate-spin' : ''}`}
                aria-hidden="true"
              />
            </motion.div>
          </AnimatePresence>
      ...
      <h3 id={titleId} className={`font-bold text-sm mt-3 ${color}`}>
        {agent.name}
      </h3>
      ...
      {agent.status === 'error' && (
        <div className="mt-2 w-full">
          <p className="text-xs text-red-300 font-mono break-all" title={agent.errorMessage}>
            {agent.errorMessage}
          </p>
        </div>
      )}
      {agent.status === 'retrying' && (
        <div className="mt-2 w-full text-center space-y-1">
          <p className="text-xs text-yellow-300 font-semibold">
            Applying fix... (Attempt {agent.retries})
          </p>
          <p
            className="text-xs text-red-400/80 font-mono break-all truncate"
            ...
            <ul className="space-y-1 text-left">
              {agent.subtasks?.map((subtask) => {
                const statusIcon = {
                  ...
                  running: (
                    <CogIcon className="h-4 w-4 text-cyan animate-spin" aria-hidden="true" />
                  ),
