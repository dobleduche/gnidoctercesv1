...
const PromptTextarea: React.FC<PromptTextareaProps> = ({
  ...
  return (
    <div
      className={`relative w-full transition-opacity duration-300 ${isGenerationDisabled ? 'opacity-60' : ''}`}
    >
      <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
        ...
        >
          {isCopied ? (
            <CheckIcon className="h-5 w-5 text-lime" />
          ) : (
      ...
      <p className="absolute bottom-3 left-6 text-xs text-gray-500" aria-hidden="true">
        Cmd/Ctrl + Enter
      </p>
