...
import { AppGenerationResult, GenerationState, FileTreeNode, TierId, FeatureFlags, Collaborator } from '../types';
...
interface ResultsDisplayProps {
  ...
  onReset: () => void;
  onUpgrade: () => void;
  scaffoldedProject: { projectId: string; projectDir: string, fileTree: FileTreeNode, files: Record<string, string> } | null;
  selectedTier: TierId;
  flags: FeatureFlags;
  buildId: string | null;
}
    ...
    return (
        <div className="w-full h-96 bg-dark-bg/50 border border-glass-border rounded-lg flex flex-col overflow-hidden shadow-lg">
            ...
            {previewUrl ? (
                ...
                <div className="flex-grow flex flex-col items-center justify-center gap-4 text-gray-500 bg-black/10" role="status">
...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ generationState, results, onReset, onUpgrade, scaffoldedProject, selectedTier, flags, buildId }) => {
    ...
    if (scaffoldedProject?.projectId && smokeTestStatus === 'idle' && buildId) {
      const runSmokeTest = async () => {
        ...
        try {
          ...
          const data = await safeFetch<{ message?: string; error?: string }>(`/api/builds/${buildId}/preview/api/test`, {
            method: 'POST',
  ...
  const renderSmokeTestStatus = () => {
    ...
    switch (smokeTestStatus) {
      case 'running':
        content = <div className="flex items-center gap-2 text-cyan"><CogIcon className="h-5 w-5 animate-spin" /><span>Running smoke test...</span></div>;
        break;
      case 'success':
        content = <div className="flex items-start gap-2 text-green-400"><CheckIcon className="h-5 w-5 mt-0.5 flex-shrink-0" /><div><p className="font-bold">Smoke Test Passed</p><p className="text-sm font-mono text-gray-300">{smokeTestResult}</p></div></div>;
        break;
      case 'error':
        content = <div className="flex items-start gap-2 text-red-400"><ErrorIcon className="h-5 w-5 mt-0.5 flex-shrink-0" /><div><p className="font-bold">Smoke Test Failed</p><p className="text-sm font-mono text-red-300">{smokeTestResult}</p></div></div>;
        break;
      default:
        content = <div className="flex items-center gap-2 text-gray-500"><DotIcon className="h-5 w-5" /><span>Pending...</span></div>;
        break;
    }
  ...
  if (generationState === GenerationState.COMPLETED && scaffoldedProject) {
    return (
      <section aria-labelledby="complete-build-title" className="w-full max-w-7xl p-6 bg-glass-bg border border-glass-border rounded-2xl backdrop-blur-lg flex flex-col items-center">
        <AnimatedShinyText className="inline-flex items-center justify-center px-4 py-1 transition ease-out hover:text-neutral-600 hover:duration-300 hover:dark:text-neutral-400">
            <h2 id="complete-build-title" className="text-2xl md:text-4xl font-orbitron font-bold text-center text-green-400 tracking-wider">BUILD COMPLETE</h2>
        </AnimatedShinyText>
        ...
        <div className="mt-8 w-full flex flex-col md:flex-row justify-center gap-4">
          <button onClick={() => handleAction('zip')} disabled={!scaffoldedProject} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-white/10 border border-glass-border rounded-lg text-white hover:bg-white/20 transition-colors disabled:opacity-50">
          ...
          <button onClick={() => handleAction('deploy')} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-cyan text-dark-bg font-bold rounded-lg hover:bg-cyan/80 transition-colors"><DeployIcon /> Deploy Now</button>
        </div>
        ...
        {githubState === 'success' && githubRepoUrl && (
            <div className="w-full text-center mt-4 p-4 bg-green-900/50 border border-green-500 rounded-lg">
                ...
                <a href={githubRepoUrl} target="_blank" rel="noopener noreferrer" className="text-cyan hover:underline font-mono">{githubRepoUrl}</a>
                <p className="text-xs text-gray-400 mt-2">(This is a simulation. In a real environment, the code would be pushed here.)</p>
            </div>
        )}
        {githubState === 'error' && (
            <div className="w-full text-center mt-4 p-4 bg-red-900/50 border border-red-500 rounded-lg" role="alert">
        ...
        {selectedTier !== 'apex' && flags.ff_payments && (
          <div className="w-full max-w-3xl mx-auto my-8 p-6 bg-dark-secondary/50 border border-cyan/50 rounded-2xl text-center shadow-lg">
              <h3 className="text-xl font-orbitron font-bold text-cyan">Supercharge Your App Quality</h3>
              <UpgradeButton onClick={onUpgrade} className="mt-6 rounded-full px-6 py-3 text-base font-bold flex items-center justify-center gap-2">
                  <SparklesIcon className="h-5 w-5" /><span className="mix-blend-multiply">Upgrade to Pro</span>
              </UpgradeButton>
          </div>
        )}
        ...
        <div className="mt-10 w-full">
            <CodeEditor
                ...
                fileTree={scaffoldedProject.fileTree}
            />
        </div>

        <div className="mt-10 w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
             <div>
                 <h3 className="flex items-center gap-2 font-bold text-magenta mb-4 text-lg"><DeployIcon /> Live Preview</h3>
            ...
            <div className="space-y-8">
                <div className="w-full p-4 bg-dark-bg/50 border border-glass-border rounded-lg">
                  <h3 className="flex items-center gap-2 font-bold text-magenta mb-3"><UsersIcon className="h-5 w-5" /> Collaboration</h3>
                  <div className="mt-2">
                    <PresenceAvatars collaborators={mockCollaborators} />
                    <ProjectChat projectId={scaffoldedProject.projectId} collaborators={mockCollaborators} />
                  </div>
                </div>
                <div className="w-full p-4 bg-dark-bg/50 border border-glass-border rounded-lg">
                  <h3 className="flex items-center gap-2 font-bold text-magenta"><BackendAgentIcon className="h-5 w-5" /> Backend Health Check</h3>
                  <div className="p-3 bg-dark-bg/80 rounded-lg mt-4 min-h-[50px] flex items-center">
                    {renderSmokeTestStatus()}
                  </div>
                </div>
                <div className="w-full p-4 bg-dark-bg/50 border border-glass-border rounded-lg">
                  <h3 className="flex items-center gap-2 font-bold text-magenta"><KeyIcon /> Required User Inputs</h3>
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 font-mono text-sm text-gray-300 mt-4">
                    {results.requiredUserInputs.map(key => <li key={key} className="flex items-center gap-2"><span className="text-cyan">{'>'}</span><span>{key}</span></li>)}
                  </ul>
                </div>
            </div>
        </div>
        
        <button onClick={onReset} className="mt-8 text-cyan hover:underline">Build Another App</button>
      </section>
    );
  }
