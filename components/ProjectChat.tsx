...
const mockInitialMessages: (collaborators: Collaborator[]) => ProjectChatMessage[] = (
  collaborators
) => [
  {
...
const ProjectChat: React.FC<ProjectChatProps> = ({ projectId, collaborators }) => {
  ...
  const [messages, setMessages] = useState<ProjectChatMessage[]>(() =>
    mockInitialMessages(collaborators)
  );
    ...
    <div className="mt-4 h-96 bg-dark-secondary/50 rounded-lg flex flex-col border border-glass-border">
      <div className="flex-grow p-4 overflow-y-auto no-scrollbar space-y-4">
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`flex items-start gap-3 ${msg.user.id === currentUser.id ? 'flex-row-reverse' : ''}`}
          >
            <div
            ...
            <div
              className={`max-w-[80%] px-3 py-2 rounded-lg ${msg.user.id === currentUser.id ? 'bg-magenta text-white' : 'bg-dark-bg text-gray-200'}`}
            >
      ...
      <div className="p-2 border-t border-glass-border">
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          ...
          <button
            ...
            className="h-9 w-9 flex-shrink-0 bg-cyan text-dark-bg rounded-full flex items-center justify-center disabled:opacity-50 transition-opacity"
          >
