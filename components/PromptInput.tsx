...
const PromptInput: React.FC<PromptInputProps> = ({ 
  prompt, setPrompt, onGenerate, isLoading, availableModels, selectedModel, setSelectedModel, fetchAvailableModels, flags,
  includeAuth, setIncludeAuth, includeBilling, setIncludeBilling, setTarget, setStack
}) => {
  ...
  if (!flags.ff_ai_features) {
    return (
      <div className="w-full max-w-3xl mx-auto flex flex-col items-center p-8 bg-glass-bg border border-yellow-500/50 rounded-2xl">
        ...
        <p className="mt-2 text-gray-400 text-center">The AI generation features are currently turned off by an administrator.</p>
      </div>
    );
  }
          ...
          >
            <div className="w-full max-w-4xl mx-auto my-6">
              {availableModels.length === 0 && (
                <div className="text-center p-4 mb-4 bg-yellow-900/50 border border-yellow-500 rounded-lg" role="alert">
                    <p className="text-yellow-300">Generation is disabled. Please configure at least one AI provider API key in your <code>.env</code> file to get started.</p>
                </div>
              )}
