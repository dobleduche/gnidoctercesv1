...
const AgentPipeline: React.FC<AgentPipelineProps> = ({ agentStatuses, commitHistory, generationState, onReset, onAgentClick }) => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-12">
        ...
        <div className="relative w-full h-full flex justify-center items-center">
          ...
          >
            <div className="w-16 h-16 md:w-24 md:h-24 bg-cyan/20 rounded-full flex items-center justify-center">
              <svg viewBox="0 0 100 100" className="w-12 h-12 md:w-16 md:h-16 text-cyan animate-spin" style={{animationDuration: '10s'}} aria-hidden="true">
                  ...
                  <text fill="currentColor" fontSize="11">
                      <textPath href="#circlePath">
                          BUILD PROCESS ACTIVE - EXECUTING -
                      </textPath>
                  </text>
              </svg>
            </div>
      ...
      {generationState === GenerationState.ERROR && (
        <div className="text-center p-4 bg-red-900/50 border border-red-500 rounded-lg" role="alert">
          <p className="text-red-300 mb-4">The build process encountered an unrecoverable error.</p>
          <button onClick={onReset} className="px-6 py-2 bg-red-500/20 text-red-200 rounded-md hover:bg-red-500/40 transition-colors font-bold">
            Start Over
          </button>
        </div>
      )}
