...
const Hero: React.FC<HeroProps> = ({ onStartTour, onUpgrade, isGenerating, children, flags }) => {
  return (
    <section
      className={`flex flex-col items-center justify-center text-center py-20 transition-all duration-500 ${isGenerating ? 'min-h-[auto] md:pt-24 md:pb-12' : 'min-h-[calc(100vh-80px)]'}`}
    >
      ...
      <p
        className={`max-w-2xl mx-auto mt-6 text-lg md:text-xl text-gray-600 dark:text-gray-400 transition-opacity duration-500 ${isGenerating ? 'opacity-0 h-0 invisible' : 'opacity-100'}`}
      >
        gnidoC terceS is a no-code AI app builder that turns your ideas into full-stack, secure, and
        shippable mobile or web applications.
      </p>
      <div
        className={`mt-10 flex flex-col sm:flex-row items-center gap-4 transition-opacity duration-500 ${isGenerating ? 'opacity-0 h-0 invisible' : 'opacity-100'}`}
      >
        {flags.ff_payments && (
