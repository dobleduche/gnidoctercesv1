...
const keyInfo = {
  gemini: { name: 'Google Gemini', Icon: GeminiIcon, envVar: 'API_KEY' },
  ...
  qwen: { name: 'Qwen (Alibaba)', Icon: QwenIcon, envVar: 'QWEN_API_KEY' },
...
const APIKeyConfigurator: React.FC = () => {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setError(null);
    try {
      const { response, data } = await safeFetch<{ ok: boolean; status: KeyStatus; error?: string }>('/api/ai/key-status');
        ...
        <div className="p-4 md:p-6">
          <div className="flex items-start gap-3 p-4 rounded-lg bg-dark-bg/50 border border-glass-border mb-6">
            ...
            <div>
              <p className="text-sm text-gray-300 font-semibold">
                This app requires API keys to be configured as environment variables on the server.
              </p>
              <p className="text-xs text-gray-400 mt-1">
                Your keys are never exposed to the frontend. Create a <code className="bg-gray-700 p-1 rounded-sm text-xs">.env</code> file in the server directory to get started.
              </p>
            </div>
          </div>
          <div className="space-y-3">
            {(Object.keys(keyInfo) as Array<keyof typeof keyInfo>).map((key) => {
              ...
              return (
                <div key={key} className={`flex items-center justify-between p-3 rounded-lg border ${isConfigured ? 'bg-green-500/10 border-green-500/30' : 'bg-yellow-500/10 border-yellow-500/30'}`}>
                  ...
                  <div className={`flex items-center gap-2 px-2.5 py-1 text-xs font-bold rounded-full ${isConfigured ? 'bg-green-500/20 text-green-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
  ...
  return (
    <div className="w-full bg-glass-bg border border-glass-border rounded-xl backdrop-blur-md min-h-[150px]">
      {renderContent()}
    </div>
  );
};
