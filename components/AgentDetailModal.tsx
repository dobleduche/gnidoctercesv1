        ...
        >
          ...
          >
            ...
            <header className="flex items-center gap-4 pb-4 border-b border-glass-border">
              ...
              <div>
                <h2 id="agent-detail-title" className="text-2xl font-orbitron font-bold text-gray-100">{agent.name}</h2>
                <div className={`inline-flex items-center gap-2 px-3 py-1 mt-1 rounded-full text-sm font-semibold ${config.bgColor} ${config.color}`}>
                  <config.Icon className={`h-4 w-4 ${agent.status === 'running' || agent.status === 'retrying' ? 'animate-spin' : ''}`} />
            ...
            <div className="flex-grow overflow-y-auto no-scrollbar mt-4 pr-2">
              ...
              {(agent.status === 'error' || agent.status === 'retrying') && agent.errorMessage && (
                <div className="mb-6 p-4 bg-red-900/50 border border-red-500 rounded-lg">
                  ...
                  <p className="mt-2 font-mono text-sm text-red-400 break-words">{agent.errorMessage}</p>
                  {agent.status === 'retrying' && (
                     <p className="mt-2 text-yellow-300">Attempting to fix... (Retry {agent.retries})</p>
                  )}
                </div>
              )}
              ...
              {agent.subtasks && agent.subtasks.length > 0 && (
                <div>
                  ...
                  <div className="w-full bg-dark-secondary rounded-full h-2.5 mb-4">
                    <div className="bg-cyan h-2.5 rounded-full" style={{ width: `${subtaskProgress}%`, transition: 'width 0.5s ease-in-out' }}></div>
                  </div>
                  <ul className="space-y-2">
                    {agent.subtasks.map((subtask) => {
                      ...
                      return (
                        <li key={subtask.name} className={`flex items-center gap-3 p-3 bg-dark-secondary/50 rounded-lg ${isCompleted ? 'opacity-70' : ''}`}>
                          <div className={`w-5 h-5 flex items-center justify-center flex-shrink-0 ${subtaskColor}`}>
                            <SubtaskIcon className={`h-5 w-5 ${subtask.status === 'running' ? 'animate-spin' : ''}`} />
                          </div>
                          <span className={`flex-grow text-sm ${isCompleted ? 'text-gray-400 line-through' : 'text-gray-200'}`}>
                            {subtask.name}
                          </span>
                        </li>
                      );
            ...
            <footer className="mt-6 pt-4 border-t border-glass-border">
              <button onClick={onClose} className="w-full px-4 py-2 text-sm bg-cyan text-dark-bg font-bold rounded-full hover:bg-cyan/80 transition-colors">
                Close
              </button>
            </footer>
          </motion.div>
