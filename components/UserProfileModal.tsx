...
const ActivityIcon: React.FC<{ type: Activity['type'] }> = ({ type }) => {
  ...
  return (
    <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${colorMap[type]}`}>
      {iconMap[type]}
    </div>
  );
};
          ...
          >
            ...
            <div className="flex-grow overflow-y-auto no-scrollbar mt-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-dark-secondary p-4 rounded-lg text-center">
                  ...
                  {currentTierInfo && (
                    <div
                      className={`inline-flex items-center gap-2 px-3 py-1 mt-1 rounded-full text-base font-bold ${currentTierInfo.bgColor}/20 ${currentTierInfo.color}`}
                    >
                ...
                <ul className="space-y-2">
                  {recentActivity.length > 0 ? (
                    recentActivity.map((activity) => (
                      <li
                        key={activity.id}
                        className="flex items-center gap-3 p-2 bg-dark-secondary/50 rounded-lg"
                      >
            ...
            <footer className="mt-6 pt-4 border-t border-glass-border">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm bg-red-500/20 text-red-300 font-bold rounded-full hover:bg-red-500/40 transition-colors"
              >
