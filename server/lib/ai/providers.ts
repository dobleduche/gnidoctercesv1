...
export class AIClients {
  ...
  hasOpenAI() { return !!this.env.OPENAI_API_KEY; }
  hasAnthropic() { return !!this.env.ANTHROPIC_API_KEY; }
  hasGemini() { return !!this.env.API_KEY; }
  hasDeepseek() { return !!this.env.DEEPSEEK_API_KEY; }
  hasQwen() { return !!this.env.QWEN_API_KEY; }
  hasXai() { return !!this.env.XAI_API_KEY; }

  async run(req: AIRequest): Promise<AIResponse> {
    try {
        switch (req.model) {
        case "openai":   return await this.openai(req);
        case "anthropic":return await this.anthropic(req);
        case "deepseek": return await this.openaiCompatible(req, "https://api.deepseek.com/v1/chat/completions", this.env.DEEPSEEK_API_KEY!, "deepseek-chat");
        case "qwen":     return await this.openaiCompatible(req, "https://api.qwen.com/v1/chat/completions", this.env.QWEN_API_KEY!, "qwen-turbo");
        case "xai":      return await this.openaiCompatible(req, "https://api.x.ai/v1/chat/completions", this.env.XAI_API_KEY!, "grok-1.5-flash");
        default:         return await this.gemini(req);
        }
    } catch (error) {
  ...
  private async openaiCompatible(req: AIRequest, url: string, key: string, defaultModel: string): Promise<AIResponse> {
    const body: any = {
      model: defaultModel,
      messages: [
