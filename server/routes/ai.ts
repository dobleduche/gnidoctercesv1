...
const responseSchema = {
    type: Type.OBJECT,
    properties: {
        goals: { type: Type.ARRAY, items: { type: Type.STRING }, description: "High-level goals of the application." },
        constraints: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Technical or business constraints for the project." },
        stack: { type: Type.ARRAY, items: { type: Type.STRING }, description: "The recommended technology stack." },
        data: { type: Type.ARRAY, items: { type: Type.STRING }, description: "A description of the data models or schema." },
        evaluation: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Key success metrics or evaluation criteria." },
        final_prompt: { type: Type.STRING, description: "The final, detailed prompt for the build agents." },
    },
...
async function refinePrompt(userText: string, models: string[], model: AIModel): Promise<RefinedPrompt> {
...
export async function aiRoute(input: Omit<AIRequest, "model"> & { model?: AIRequest["model"] }) {
  const chosen = input.model ?? (DETECT_ORDER.find(m =>
...
ai.get("/available-models", (_req: express.Request, res: express.Response) => {
    try {
        const available = DETECT_ORDER.filter(m => 
            (m === "gemini" && clients.hasGemini()) ||
            (m === "openai" && clients.hasOpenAI()) ||
            (m === "anthropic" && clients.hasAnthropic()) ||
...
ai.get("/list-gemini-models", async (_req: express.Request, res: express.Response) => {
    try {
        ...
        if (!apiKey) {
            return res.status(500).json({ ok: false, error: "Google AI API key is not configured on the server." });
        }
...
ai.post("/refine", async (req: express.Request, res: express.Response) => {
    try {
        ...
        const availableModels = DETECT_ORDER.filter(m => 
            (m === "gemini" && clients.hasGemini()) ||
            (m === "openai" && clients.hasOpenAI()) ||
            (m === "anthropic" && clients.hasAnthropic())
        );
...
ai.post("/orchestrate", async (req: express.Request, res: express.Response) => {
    try {
        ...
        const modelsToRun = models && Array.isArray(models) 
